...
const BubbleChart = (): JSX.Element => {
  ...
  React.useEffect(() => {
    ...
    if (d3Container.current) {
      ...
      const year1 = [
        {
          ...
          measure: 400,
          property: "This is an awesome Game!",
        },
        {
          title: "Minecraft",
          measure: 100,
      ...
      const year2 = [
        { title: "1", measure: 400, property: "This is an awesome Game!" },
      ...
      const pack = function (size, pack_padding) {
        return d3.pack().size(size).padding(30);
      };
      ...
      const createBubbles = function (data) {
        ...
        leaf
          ...
          .selectAll("tspan")
          .data((d) =>
            d.data.measure >= minMeasureForText
      ...
      const createSlider = function () {
        ...
        var sliderSimple = sliderBottom()
          ...
          .on("onchange", (val) => {
            ...
            d3.select("p#bubble-slider-text").text(
              `${val.getUTCMonth() + 1}:${val.getUTCFullYear()}`
            );
            ...
            if (val.getUTCFullYear() != currentlyDisplayedYear) {
              ...
              const temp_data = makeHierarchicalData(
                eval("data.year" + dataIndx)
              ).leaves();
              var group = svg.selectAll("g").data(temp_data);
              group.exit().remove(); //
              group.enter().append("circle").attr("r", 0); //create any new circles needed
